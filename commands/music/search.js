...
if (!args[0]) return message.channel.send(`${message.author}, Please enter a valid song name. ❌`);
        ...
        embed.setDescription(`${maxTracks.map((track, i) => `**${i + 1}**. ${track.title} | ${track.author}`).join('\n')}\n\nChoose a song from **1** to **${maxTracks.length}** or say **cancel** to cancel the selection`);
       ...
       collector.on('collect', async (query) => {
            if (query.content.toLowerCase() === 'cancel') return message.channel.send(`Selection Has Been Cancelled.`) && collector.stop();
        ...
        collector.on('end', (msg, reason) => {
            if (reason === 'time') return message.channel.send(`${message.author}, Search time expired 😔`);
        });
    },
};